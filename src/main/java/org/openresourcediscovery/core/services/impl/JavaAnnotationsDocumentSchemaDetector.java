...
public class JavaAnnotationsDocumentSchemaDetector implements DocumentSchemaDetector {
  ...
  public Map<String, Pair<DocumentSchema, Set<String>>> detect(OrdProperties ordProperties) {
    Map<String, Pair<DocumentSchema, Set<String>>> documents = lookupDocuments();
  ...
  protected Map<String, Pair<DocumentSchema, Set<String>>> lookupDocuments() {
    ...
    ordAnnotationsScanner
        ...
        .forEach(d -> documents.put(
            d.getRight().id(),
            ImmutablePair.of(
                generator.generate(Context.of(d.getRight(), d.getLeft(), null)),
                Stream.of(d.getRight().accessStrategies())
