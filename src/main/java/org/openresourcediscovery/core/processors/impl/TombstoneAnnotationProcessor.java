...
public class TombstoneAnnotationProcessor implements AnnotationProcessor<Ord.Tombstone, Tombstone> {
  ...
  private ObjectProvider<Customizer<Ord.Tombstone>> customizers;
  ...
  public void process(Map<String, DetectionResult> documents) {
    ...
    ordAnnotationsScanner.scan(Ord.Tombstone.class).forEach(t -> {
      ...
      document.setTombstones(ListUtils.union(
          ...
          List.of(entityGenerator.generate(Context.of(t.annotation(), t.annotated(), document)))));

      Optional.ofNullable(customizers)
          .map(ObjectProvider::orderedStream)
          .orElse(Stream.empty())
          .forEach(customizer -> customizer.customize(t.annotation(), document));
    });
  }
}
