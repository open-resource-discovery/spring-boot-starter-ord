...
class OrdDocumentBuildVsRuntimeTest {
  ...
  private OrdAuthorizationManager ordAuthorizationManager;
  ...
  void setUp() {
    ...
    ordAuthorizationManager = mock(OrdAuthorizationManager.class);
  ...
  void runtime_open_snapshot() throws Exception {
    var result = standaloneSetup(
            new OpenResourceDiscoveryController(documentSchemaRegistry, ordAuthorizationManager))
        .build()
  ...
  void runtime_basic_snapshot() throws Exception {
    ...
    when(ordAuthorizationManager.isAuthenticated(any())).thenReturn(Boolean.TRUE);
